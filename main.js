/* ============================================================================
   ...
   from live MyGeotab data plus user-entered rates:
     ...
     - ChargeEvent   completed charging sessions in the period
                       .measuredEnergyConsumption -> kWh billed (per SDK)
                       .peakPowerKw               -> AC vs DC-fast heuristic
...
(function () {
  ...
  var DEFAULT_RATES = {
    ...
    dieselPrice: 0,        // € / litre
    gasolinePrice: 0,      // € / litre
    adbluePrice: 0,        // € / litre (diesel only; ~4% of diesel volume)
    fuelConsumption: 0,    // L / 100 km
    leaseMonthly: 0,
    roadTax: 0,
  ...
  // Energy billed for a charge session. Per the SDK this is
  // measuredEnergyConsumption (kWh); confirm on a live database with GetEntity.
  function energyOf(ce) {
    return typeof ce.measuredEnergyConsumption === "number" ? ce.measuredEnergyConsumption : 0;
  }
  ...
  geotab.addin.tcoCostDashboard = function () {
    ...
    function loadAndRender(api) {
      ...
      api.multiCall([
        ...
        try {
          var model = buildModel(r[0] || [], r[1] || [], r[2] || [], r[3] || [], period);
          render(model);
          setSync("synced");
    ...
    // ---- Model ---------------------------------------------------------
    function buildModel(devices, socRows, chargeEvents, trips, period) {
      ...
      // include fossil-fuel vehicles (no ChargeEvent) that drove in the period
      Object.keys(tripAgg).forEach(function (id) {
        if (isFossilType(resolveRates(id).rates.fuelType)) ids[id] = true;
      });
      ...
      var vehicles = Object.keys(ids).map(function (id) {
        ...
        var c = costOf(resolved.rates, evs, overrides, agg.km, periodDays);
        return {
          id: id,
          name: d.name || id,
      ...
      return { vehicles: vehicles, period: period, periodDays: periodDays, vehicleRateCount: vehicleRateCount };
    }
    ...
    function renderTable(model) {
      ...
      el.tcoTableBody.innerHTML = v.map(function (row) {
        ...
        return '<tr>' +
          '<td>' + vehGlyph(row) + '<a class="tco-veh-link" data-open-device data-id="' + escapeHtml(row.id) + '">' + escapeHtml(row.name) + '</a>' + fb +
            (estimated ? '<span class="tco-est-flag" title="' + (LANG === "nl" ? "Gebruikt standaardtarieven" : "Using fleet default rates") + '">' + t("est") + '</span>' : '') + '</td>' +
    ...
    function rebuildFromLastData() {
      ...
      lastModel.vehicles.forEach(function (v) {
        ...
        var c = costOf(resolved.rates, v.chargeEvents, overrides, v.distanceKm, lastModel.periodDays);
        ...
        v.rates = resolved.rates; v.overriddenKeys = keys;
    ...
    function copyCsv() {
      ...
      var rows = lastModel.vehicles.map(function (v) {
        return [v.name, FUEL_LABELS[v.fuelType] || "Electric", FINANCING_LABELS[v.mode] || v.mode, v.tripCount, Math.round(v.distanceKm),
    ...
    function mockModel(period) {
      ...
      var vehicles = profiles.map(function (p) {
        ...
        var c = costOf(resolved.rates, evs, overrides, distanceKm, days);
        ...
        return {
          ...
          rates: resolved.rates, overriddenKeys: seededKeys,
      ...
      vehicles.sort(function (a, b) { return b.tco - a.tco; });
      return { vehicles: vehicles, period: period, periodDays: days, vehicleRateCount: Object.keys(readJson(LS_VEHICLE, {})).length };
    }
    ...
    function bindOnce() {
      ...
      document.addEventListener("keydown", function (e) {
        if (e.key === "Escape") { el.tcoModal.hidden = true; el.tcoDefaultsModal.hidden = true; el.tcoScenarioModal.hidden = true; if (el.tcoRoadModal) el.tcoRoadModal.hidden = true; modalState = null; }
      });
    }
